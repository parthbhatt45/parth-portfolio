...
function CertCard({ cert, index, visible }) {
    ...
    return (
        <div
            ...
            style={{
                ...
                transform: visible
                    ...
                    : "translateY(28px)",
                borderColor: hovered ? `${cert.accent}35` : "rgba(255,255,255,.07)",
                background: hovered ? "var(--bg-card2)" : "var(--bg-card)",
                boxShadow: hovered ? "0 14px 40px rgba(0,0,0,.45)" : "none",
                transition: `opacity .55s cubic-bezier(.16,1,.3,1) ${index * .08}s,
                     transform .25s ease,
        ...
        >
            ...
            <div style={S.body}>
                {/* Number badge */}
                <div style={{ ...S.numBadge, background: `${cert.accent}14`, color: cert.accent }}>
                    {String(index + 1).padStart(2, "0")}
                </div>

                <p style={{
                    ...S.title,
                    color: hovered ? "#fff" : "var(--text-main)",
...
export default function Certifications() {
    ...
    return (
        <section id="certifications" ref={ref} style={S.section}>
            <div style={{
                opacity: visible ? 1 : 0, transform: visible ? "translateY(0)" : "translateY(32px)",
                transition: "opacity .6s cubic-bezier(.16,1,.3,1), transform .6s cubic-bezier(.16,1,.3,1)",
            }}>
                <p className="section-label">Credentials</p>
                <h2 style={S.heading}>Certifications</h2>
            </div>

            <div style={S.grid}>
                {CERTS.map((c, i) => (
                    <CertCard key={c.title} cert={c} index={i} visible={visible} />
                ))}
            </div>
        </section>
    );
...
const S = {
    ...
    body: { display: "flex", alignItems: "flex-start", gap: "16px", padding: "22px 22px 24px" },
    numBadge: {
